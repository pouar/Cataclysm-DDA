...
void player::update_needs()
{
    ...
    // Check if need to pee
        if (bladder >= bladderdance) {
            if (bladder >= bladdercap || has_trait("INCONT") || has_effect("sleep") || has_effect("lying_down")) {
                peeself(false);
    ...
    if (calendar::turn % peerate == 0 && !has_effect("sleep")) {
        bladder++;
        if(has_effect("diuretic"))
            bladder++;
    }
    else if (calendar::turn % peesleeprate == 0 && has_effect("sleep")) {
        bladder++;
        if(has_effect("diuretic"))
            bladder++;
    }
