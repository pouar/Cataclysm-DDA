...
std::string item::info( bool showtext, std::vector<iteminfo> &info ) const
{
    ...
    if( is_armor() ) {
        ...
        info.push_back( iteminfo( "ARMOR", _( "Absorbancy: " ), "<num> ml", type->peecap, true, "", false));
        info.push_back( iteminfo( "ARMOR", _( "Usage: " ), "<num> ml", pee, true, "", true));
    ...
    if( showtext && !is_null() ) {
        ...
        if( is_armor() && has_flag( "FIT" ) ) {
            ...
            info.push_back(iteminfo("DESCRIPTION", "--"));
            info.push_back(iteminfo("DESCRIPTION", _("There's a lock on it that prevents you from removing it.")));
        }
        if (is_armor() && has_flag("DIAPER")) {
            info.push_back(iteminfo("DESCRIPTION", "--"));
            info.push_back(iteminfo("DESCRIPTION", _("This piece of clothing is absorbant enough to be used as a toilet.")));
            if (pee>0) {
                info.push_back(iteminfo("DESCRIPTION", "--"));
                if(pee>=type->peecap && !has_flag("EVERDIAPER"))
                    info.push_back(iteminfo("DESCRIPTION", _("Urine is leaking from it.")));
                else if(pee>type->peecap*.75)
                    info.push_back(iteminfo("DESCRIPTION", _("It squishes when you press it.")));
                else if(pee>type->peecap*.25)
                    info.push_back(iteminfo("DESCRIPTION", _("It is soaked in urine, but it can hold more.")));
                else
                    info.push_back(iteminfo("DESCRIPTION", _("It is barely wet.")));
            }
        }
        if (is_armor() && !has_flag("DIAPER")) {
            if (pee>0) {
                info.push_back(iteminfo("DESCRIPTION", "--"));
                if(pee<type->peecap)
                info.push_back(iteminfo("DESCRIPTION", _("It is Damp with urine.")));
                else
                info.push_back(iteminfo("DESCRIPTION", _("It is drenched in urine.")));
            }
        }
