...
int iuse::bladderstim(player *p, item *it, bool, const tripoint& )
{
    if (!p->has_effect("diuretic")) {
        p->add_effect("diuretic", 900);
    } else {
        p->add_effect("diuretic", 200);
    }
    return it->type->charges_to_use();
}

int iuse::bladderset(player *p, item *it, bool, const tripoint& )
{
...
int iuse::diaperlock(player *p, item *it, bool, const tripoint& )
{
    if (p->is_underwater()) {
        p->add_msg_if_player(m_info, _("You can't do that while underwater."));
        return 0;
...
int iuse::combatsaw_off(player *p, item *it, bool, const tripoint& )
{
    return toolweapon_off(p, it,
        true,
        !p->is_underwater(),
        30, _("If you want to live you have to kill or become."),
...
int iuse::diaperfur_whistle(player *p, item *it, bool, const tripoint& )
{
    if (p->is_underwater()) {
        p->add_msg_if_player(m_info, _("You can't do that while underwater."));
        return 0;
    ...
    if (mc->get_var( "MC_PHOTOS", 0 ) > 0) {
        ...
        p->add_msg_if_player(m_good, string_format(
                                 ngettext("You transfered %d new photo to the laptop's hdd.",
                                          "You transfered %d new photos to the laptop's hdd.", new_photos),
                                                   new_photos).c_str());
    ...
    if (mc->get_var( "MC_MUSIC", 0 ) > 0) {
        ...
        p->add_msg_if_player(m_good, string_format(
                                 ngettext("You transfered %d new song to the laptop's hdd.",
                                          "You transfered %d new songs to the laptop's hdd.", new_songs),
                                                   new_songs).c_str());
